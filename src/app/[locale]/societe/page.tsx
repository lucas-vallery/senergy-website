...
export default async function CompanyPage({ params }: { params: { locale: string } }) {
  ...
  const values = [
    { icon: <Target size={24} className="text-orange" />, title: t('value1Title'), desc: t('value1Desc') },
    { icon: <Heart size={24} className="text-orange" />, title: t('value2Title'), desc: t('value2Desc') },
    { icon: <Zap size={24} className="text-orange" />, title: t('value3Title'), desc: t('value3Desc') },
    { icon: <Clock size={24} className="text-orange" />, title: t('value4Title'), desc: t('value4Desc') },
  ];
                ...
                {[
                  ...
                  <div key={i} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${item.current ? 'bg-orange text-white' : 'bg-navy/10 text-navy'}`}>
                        {i + 1}
                      </div>
                      {i < 3 && <div className="w-0.5 h-full bg-gray-100 mt-2" />}
                    </div>
                    <div className="pb-6">
                      <span className="text-xs font-semibold text-orange uppercase tracking-wider">{item.year}</span>
                      <h4 className="font-barlow font-semibold text-navy text-sm mt-0.5">{item.label}</h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
              {values.map((val, i) => (
                <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-orange/30 hover:shadow-md transition-all text-center">
                  ...
                  <h3 className="font-barlow font-semibold text-navy mb-2">{val.title}</h3>
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {team.map((member) => (
                <div key={member.email} className="bg-lightbg rounded-2xl p-6 text-center border border-gray-100 hover:border-navy/20 hover:shadow-md transition-all">
                  ...
                  <h3 className="font-barlow font-semibold text-navy text-lg">{member.name}</h3>
                  <p className="text-sm text-orange font-medium mt-1 mb-3">{member.role}</p>
                  <a
                    href={`mailto:${member.email}`}
                    className="text-xs text-gray-500 hover:text-navy transition-colors break-all"
                  >
                    {member.email}
                  </a>
                </div>
