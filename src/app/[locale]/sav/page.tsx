...
export default async function SAVPage({ params }: { params: { locale: string } }) {
  ...
  const services = [
    { icon: <Zap size={24} className="text-orange" />, title: t('service1Title'), desc: t('service1Desc') },
    { icon: <Shield size={24} className="text-orange" />, title: t('service2Title'), desc: t('service2Desc') },
    { icon: <Package size={24} className="text-orange" />, title: t('service3Title'), desc: t('service3Desc') },
    { icon: <Headphones size={24} className="text-orange" />, title: t('service4Title'), desc: t('service4Desc') },
  ];

  const commitments = [
    t('commitment1'), t('commitment2'), t('commitment3'), t('commitment4'),
  ];
    ...
    <>
      ...
      <main className="pt-[104px]">
        <Hero
          ...
          ctaPrimary={{ label: tCommon('contactUs'), href: '../contact' }}
          pattern="page"
          size="medium"
        />
          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              <div>
                ...
                <div className="bg-lightbg rounded-2xl p-6 border border-gray-100">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 bg-navy rounded-full flex items-center justify-center flex-shrink-0">
                      <span className="font-barlow font-bold text-white text-xl">BL</span>
                    </div>
                    <div>
                      <p className="font-barlow font-bold text-navy">{t('contactSAV')}</p>
                      <p className="text-orange text-sm font-medium mb-2">{t('contactSAVRole')}</p>
                      <a href={`mailto:${contactSAVEmail}`} className="text-sm text-gray-500 hover:text-navy transition-colors flex items-center gap-1.5">
                        <Mail size={13} />
                        {contactSAVEmail}
                      </a>
                    </div>
              ...
              <div className="bg-navy rounded-2xl p-8 text-white">
                ...
                <h3 className="font-barlow font-bold text-2xl mb-3">{t('hotlineTitle')}</h3>
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{hotlineEmail}</p>
                    </div>
                  </a>
                </div>
        ...
        <section className="py-20 bg-lightbg">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
              {services.map((s, i) => (
                <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-orange/30 hover:shadow-md transition-all text-center">
                  <div className="w-12 h-12 bg-orange/10 rounded-xl flex items-center justify-center mx-auto mb-4">
                    {s.icon}
                  </div>
                  <h3 className="font-barlow font-semibold text-navy mb-2">{s.title}</h3>
        ...
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
              {commitments.map((c, i) => (
                <div key={i} className="flex items-center gap-3 bg-lightbg rounded-xl px-5 py-4 border border-gray-100">
                  <div className="w-8 h-8 bg-orange rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-white font-bold text-sm">{i + 1}</span>
                  </div>
                  <span className="text-sm font-medium text-textdark">{c}</span>
                </div>
              ))}
            </div>
          </div>
